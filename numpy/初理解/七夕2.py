...
class MessageBox(QtWidgets.QWidget):  # 继承自父类QtWidgets.QWidget
    ...
    def __init__(self, parent=None):  # parent = None代表此QWidget属于最上层的窗口,也就是MainWindows.
        QtWidgets.QWidget.__init__(self)  # 因为继承关系，要对父类初始化通过super初始化父类，__init__()函数无self，若直接QtWidgets.QWidget.__init__(self)，括号里是有self的
        ...
        self.setWindowTitle(u'小姐姐') #仅仅设置窗体标题，不设置位置。
        self.setWindowIcon(QIcon('E:/初理解/rose.png')) #  调用QIcon构造函数时，我们需要提供要显示的图标的路径(相对或绝对路径)。同时注意：使用QIcon类型必须导入此模块from PyQt5.QtGui import QIcon
        self.setToolTip(u'这里有一只小哥哥')  # 调用setToolTip()方法,该方法接受富文本格式的参数,css之类。
        ...
        self.label2.move(80,100)#  使此标签移动到这个部件(260,100)的位置
        ...
        self.window_pale.setBrush(self.backgroundRole(),QtGui.QBrush(QtGui.QPixmap("E:/初理解/rose.png")))  # 打开图片
    ...
    def closeEvent(self, event):  # 函数名固定不可变
        ...
        if self.CloseAllowed == 1:
            ...
            QtWidgets.QMessageBox.information(self,"嗯哼","小姐姐,请不要逃避!",QtWidgets.QMessageBox.Ok)
